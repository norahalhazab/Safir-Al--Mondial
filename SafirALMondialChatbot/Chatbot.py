...
# ========== تحميل البيانات ==========
data_path = r'C:\Users\norah\PycharmProjects\pythonProject3'

csv_files = {
    'fan_preferences': 'fan_preferences.csv',
    'museums_data': 'List_of_public_and_private_museums.csv',
    'matches': 'matches.csv',
...
def get_fan_events(city, style):
    df = dataframes.get('fan_preferences')
    if df is not None:
        events = df[
...
def get_museums(province, typology):
...
def chat_response():
    ...
    # -------- Museums --------
        ...
        province = extract_value(user_msg, 'in')
        typology = extract_value(user_msg, 'type')
        df = get_museums(province, typology)
        if not df.empty:
            response = f"🏛 Museums in {province.title()}:\n" + '\n'.join(
                f"- {row['asset name']} ({row['organisational status']})" for _, row in df.iterrows()
            )
        else:
